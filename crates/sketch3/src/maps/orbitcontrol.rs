            ...
            match event {
                Event::MouseMotion {
                    ...
                    if !*handled && Some(MouseButton::Left) == *button {
                        let speed = 0.01 * ((self.target.distance(camera.position()) - self.min_distance) / (self.max_distance - self.min_distance));
                ...
                Event::MouseWheel { delta, handled, .. } => {
                    if !*handled {
                        let speed = 0.01 * (self.target.distance(camera.position()) - self.min_distance) + 0.001;
                        camera.zoom_towards(
                            self.target,
                            speed * delta.1,
                            self.min_distance,
                ...
                Event::PinchGesture { delta, handled, .. } => {
                    if !*handled {
                        let speed = (self.target.distance(camera.position()) - self.min_distance) + 0.1;
                        camera.zoom_towards(
                            self.target,
                            speed * *delta,
                            self.min_distance,
